...
const getOne = (Model) =>
  catchAsync(async (req, res, next) => {
    ...
    const doc = await Model.findById(req.params.id).select(selectFields).exec();
...
const getOneFunc = tryCatch(async (Model, id, selectFields) => {
  selectFields = selectFields && selectFields.split(',');

  const doc = await Model.findById(id).select(selectFields).exec();

  if (!doc) return new Error('doc does not exist');
...
const getAllFunc = tryCatch(async (Model, query, populateOptions) => {
  const findQuery = Model.find();
  if (populateOptions) query = query.populate(populateOptions);

  const apiObj = new ApiFeatures(findQuery, query)
    .filter()
    .sort()
    .pagination()
    .limitFields();
  ...
  if (!docs) return new Error('docs not found');
  return docs;
});
...
const getAllDocsCount = (Model) =>
  catchAsync(async (req, res, next) => {
    ...
    const apiObj = new ApiFeatures(query, req.query).filter().sort();
...
const getAllDocsCountFunc = tryCatch(async (Model, reqQuery) => {
  ...
  const apiObj = new ApiFeatures(query, reqQuery).filter().sort();
...
const updateOneFunc = tryCatch(async (Model, id, body) => {
  ...
  if (!doc) return new Error('No doc found with this id');
  return doc;
});
...
const deleteOneFunc = tryCatch(async (Model, id) => {
  const doc = await Model.findOneAndDelete({ _id: id }).exec();
  if (!doc) return new Error('No doc found with this id');
});
